...
const SHOP_MULTIPLIERS = {
  COMMON: 1, RARE: 1.2, EPIC: 1.5, LEGENDARY: 2,
  MYTHIC: 2.5, SECRET: 3, TRANSCENDENTAL: 3.5, OMNIVERSAL: 4,
};

const SHOP_STOCK = {
  COMMON: -1, RARE: -1, EPIC: 5, LEGENDARY: 3,
  MYTHIC: 1, SECRET: 1, TRANSCENDENTAL: 1, OMNIVERSAL: 1,
};
    ...
    for (const itemData of ITEMS) {
      ...
      await prisma.shopListing.create({
        data: {
          itemId: item.id,
          price: shopPrice,
          stock: SHOP_STOCK[itemData.rarity],
        },
      });
    }

    const adminHash = await bcrypt.hash("Bobbybenson321!", 12);
